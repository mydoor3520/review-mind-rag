...
def search_product_reviews(product_id: str, k: int = 30) -> Tuple[List[Any], Optional[str]]:
...
def generate_summary(documents: List[Any]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
...
def extract_pros_cons(documents: List[Any]) -> Tuple[Optional[Dict[str, List[str]]], Optional[str]]:
    summarizer, error = get_summarizer()
    if summarizer is None:
        return None, error
...
def analyze_sentiment(documents: List[Any]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
            ...
            if search_error:
                ...
                with col2:
                    ...
                    if sent_error:
                        ...
                        st.metric("⭐ 평균 평점", f"{sentiment_result['average_rating']}점")
                ...
                if sum_error:
                    ...
                    st.caption(f"분석된 리뷰: {summary_result['review_count']}개 / 총 {summary_result.get('total_available', len(documents))}개")
    else:
